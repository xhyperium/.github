...
> 适用范围：所有 Agent Teams 会话（claude-teams MCP），所有项目
> 级别：🔥 P0（强制）
> 互补规则：[agent-model-routing.md](./agent-model-routing.md)（模型路由）、[agent-discipline.md](./agent-discipline.md)（执行纪律）

---

## 1. 波次调度（P0 铁律）

所有多任务执行**必须**按波次（Wave）组织，禁止一次性 spawn 全部 agent。
...
- 团队名称必须语义化（如 `infra-v11-wave1`），禁止随机名
- 每个 teammate 的 `model` 统一使用 **opus**，不做成本分级
