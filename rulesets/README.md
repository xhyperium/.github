...
├── rust/                                 # Rust 全局规范完整版 v2.1.2
│   ├── RULES.md                          # 入口
│   ├── ci.md                             # 门禁落地矩阵 + 新仓接入
│   └── deny.template.toml                # cargo-deny 组织起点模板
...
| [`check-workspace-deps.sh`](../scripts/check-workspace-deps.sh) | R-DEP-004：成员禁止内联第三方 `version` |
...
| 入口 | `rulesets/rust/RULES.md`（v2.1.2） |
| 关系 | 项目可加严，**不可削弱** P0 |
| 落地矩阵 | [rust/ci.md §0](./rust/ci.md)（规范 P0 ≠ Org Ruleset 硬拦） |
...
- [ ] 改 P0 规则走 PR
- [ ] Rust 专题版本头与 `RULES.md` 主版本一致（当前 2.1.2）
