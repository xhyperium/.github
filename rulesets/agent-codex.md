...
Worker 命令白名单：按语言默认可跑验证命令（Rust: `cargo fmt`/`clippy`/`test`；Go: `gofmt`/`go test`/`go vet`/`go build`）。详见 [agent-quality-gates.md](./agent-quality-gates.md)。其他命令需主 Agent 明确授权。
